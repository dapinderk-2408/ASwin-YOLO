...
class WindowAttention_v2(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if(max.device != self.logit_scale.device):
            self.logit_scale=self.logit_scale.to('cuda')
            max=max.to('cuda')
...
class SwinTransformer2Block(nn.Module):
    def __init__(self, c1, c2, num_heads, num_layers, window_size=4):
        super().__init__()
        self.conv = None
        if c1 != c2:
            self.conv = Conv(c1, c2)
...
class EECA(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, reductions=16, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        #self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        #self.bn = nn.BatchNorm2d(c2)
        #self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        ...
        self.conv1 = nn.Conv1d(c1, c2//reductions, k, s, autopad(k, p), groups=g, bias=False)
        self.conv2 = nn.Conv1d(c2//reductions, c1, k, s, autopad(k, p), groups=g, bias=False)
    ...
    #SCEECA
class SCEECA(nn.Module):

    def __init__(self, inplans, planes, conv_kernels=[3, 5, 7, 9], stride=1, conv_groups=[1, 4, 8, 16]):
        super(SCEECA, self).__init__()
        conv_kernels=[3, 5, 7, 9]
        self.conv_1 = conv(inplans, planes//4, kernel_size=conv_kernels[0], padding=conv_kernels[0]//2,
                            stride=stride, groups=conv_groups[0])
        self.conv_2 = conv(inplans, planes//4, kernel_size=conv_kernels[1], padding=conv_kernels[1]//2,
        ...
        self.conv_4 = conv(inplans, planes//4, kernel_size=conv_kernels[3], padding=conv_kernels[3]//2,
                            stride=stride, groups=conv_groups[3])
        self.eeca = EECA(inplans, planes // 4)
        self.split_channel = planes // 4
        self.softmax = nn.Softmax(dim=1)
    ...
    def forward(self, x):
        ...
        x1_eeca = self.eeca(x1)
        x2_eeca = self.eeca(x2)
        x3_eeca = self.eeca(x3)
        x4_eeca = self.eeca(x4)

        x_se = torch.cat((x1_eeca, x2_eeca, x3_eeca, x4_eeca), dim=1)
        attention_vectors = x_se.view(batch_size, 4, self.split_channel, 1, 1)
        attention_vectors = self.softmax(attention_vectors)
        feats_weight = feats * attention_vectors
        for i in range(4):
